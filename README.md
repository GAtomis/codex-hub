...
- 入口定位：作为中枢首页，优先做“继续工作”和项目切换，不再把管理表格放在第一屏
- 总览指标：项目数、运行中线程、已完成线程、异常线程
- 项目管理：注册/编辑项目、路径校验
- Agent 管理：可视化启动、停止、删除 Agent，并查看最近采集输出摘要
- 最近事件：保留运维视角，但降到第二层信息区

## 6.2 项目控制台 `/projects/{slug}`

- 顶部 Hero：展示当前项目状态、路径、线程数、运行中线程数
- 中间主工作区：`Codex 项目会话台` 作为绝对主位
- `Codex 项目会话台`：
...
- 右侧辅助区：
  - 查看最近线程
  - 历史记录 label 优先显示第一句用户提问
  - 快速跳转线程页
- 底部 Agent 控制台：在当前项目上下文内直接启动/停止采集 Agent

## 6.3 线程页 `/threads/{threadId}`

- 沉浸式 transcript：主区优先显示对话内容，视觉和交互更接近 `codex app`
- 在线程内继续发送 prompt（续接会话）
- 右侧线程摘要：显示第一句用户提问、最近一段 assistant 回复
- 事件时间线可折叠，避免抢占主阅读空间
- 可展开调试流（stream debug）
